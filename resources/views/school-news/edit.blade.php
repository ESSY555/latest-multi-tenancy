...
<section class="relative bg-gradient-to-br from-blue-600 via-indigo-700 to-purple-800 py-16">
    ...
    <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h1 class="text-4xl md:text-5xl font-bold text-white mb-4">Edit News Article</h1>
        <p class="text-xl text-blue-100 max-w-3xl mx-auto">
            Update and modify your news article content and publishing settings
        </p>
    </div>
</section>
...
<section class="py-12 bg-white">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-xl shadow-lg p-8">
            <form method="POST" action="{{ route('school-news.update', $schoolNews) }}" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" 
                           id="title" 
                           name="title" 
                           value="{{ old('title', $schoolNews->title) }}"
                ...
                <div>
                    ...
                    <select id="branch_id" 
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->id }}" {{ old('branch_id', $schoolNews->branch_id) == $branch->id ? 'selected' : '' }}>
                                {{ $branch->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea id="excerpt" 
                              ...
                              placeholder="A brief summary of the article (max 500 characters)">{{ old('excerpt', $schoolNews->excerpt) }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="content" 
                              ...
                              required>{{ old('content', $schoolNews->content) }}</textarea>
                ...
                @if($schoolNews->image_path)
                    <div>
                        ...
                        <div class="flex items-center space-x-4">
                            <img src="{{ Storage::disk('public')->url($schoolNews->image_path) }}" 
                                 alt="Current featured image" 
                                 class="w-32 h-24 object-cover rounded-lg border">
                            <div>
                                <p class="text-sm text-gray-600">Current image: {{ basename($schoolNews->image_path) }}</p>
                ...
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $schoolNews->image_path ? 'Replace Featured Image' : 'Featured Image' }} (Optional)
                    </label>
                    <input type="file" 
                           id="image" 
                           name="image" 
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                            <p class="text-sm text-blue-800">
                                ...
                                @if($schoolNews->is_published)
                                    <span class="text-green-600">Published</span>
                                    @if($schoolNews->published_at)
                                        on {{ $schoolNews->published_at->format('F j, Y \a\t g:i A') }}
                                    @endif
                                @else
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" 
                                   ...
                                   name="is_published" 
                                   value="1"
                                   {{ old('is_published', $schoolNews->is_published) ? 'checked' : '' }}
                        ...
                        <div id="schedule-section" class="{{ old('is_published', $schoolNews->is_published) ? 'hidden' : '' }}">
                            ...
                            <input type="datetime-local" 
                                   id="published_at" 
                                   name="published_at" 
                                   value="{{ old('published_at', $schoolNews->published_at ? $schoolNews->published_at->format('Y-m-d\TH:i') : '') }}"
                ...
                <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-200">
                    ...
                    <a href="{{ route('school-news.show', $schoolNews) }}" 
