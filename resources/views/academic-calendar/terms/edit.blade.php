                    ...
                    <div class="mt-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
                        ...
                        <div class="space-y-4">
                            <div class="bg-white border-2 border-dashed border-gray-300 rounded-lg p-2 flex flex-col items-center">
                                <canvas id="principal-signature-pad" class="w-full h-40 cursor-crosshair bg-white touch-none" style="max-width: 500px;"></canvas>
                                @if($term->principal_signature)
                                    <div class="mt-2 flex flex-col items-center">
                                        <span class="text-xs text-gray-500 mb-1">Current Signature:</span>
                                        <img src="{{ $term->principal_signature }}" alt="Principal Signature" style="max-width:200px;max-height:60px;border:1px solid #ccc;background:#fff;">
                                    </div>
                                @endif
                                <div class="w-full border-t border-gray-200 mt-2 pt-2 flex justify-between items-center px-4">
                                    ...
                                    <button type="button" onclick="clearPrincipalSignature()" class="text-xs font-bold text-red-500 hover:text-red-700 uppercase transition-colors px-2 py-1 rounded hover:bg-red-50">
                                        Clear Signature
                                    </button>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function resizePrincipalCanvas() {
            ...
            const existing = @json($term->principal_signature);
            if (existing) {
                const img = new window.Image();
                img.onload = function() {
                    principalSignaturePad.clear();
                    principalSignaturePad.fromDataURL(existing);
                };
                img.src = existing;
            } else {
                principalSignaturePad.clear();
        ...
        if (form && principalSignaturePad) {
            form.addEventListener('submit', function(e) {
                ...
                const existing = @json($term->principal_signature);
                if (principalSignaturePad.isEmpty() && !existing) {
                    e.preventDefault();
                    alert('Please provide the principal\'s signature before submitting.');
                    return;
                }
                // If signature pad is not empty and changed, update value
                if (!principalSignaturePad.isEmpty()) {
                    ...
                    document.getElementById('principal_signature_input').value = existing || '';
                }
            });
        }
    });
