...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-lg p-6">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Edit Syllabus</h1>
        ...
        <form action="{{ route('admin.syllabus.update', $syllabus->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="class" name="class" required 
                        ...
                        <option value="JSS1" {{ $syllabus->class == 'JSS1' ? 'selected' : '' }}>JSS1</option>
                        <option value="JSS2" {{ $syllabus->class == 'JSS2' ? 'selected' : '' }}>JSS2</option>
                        <option value="JSS3" {{ $syllabus->class == 'JSS3' ? 'selected' : '' }}>JSS3</option>
                        <option value="SS1" {{ $syllabus->class == 'SS1' ? 'selected' : '' }}>SS1</option>
                        <option value="SS2" {{ $syllabus->class == 'SS2' ? 'selected' : '' }}>SS2</option>
                        <option value="SS3" {{ $syllabus->class == 'SS3' ? 'selected' : '' }}>SS3</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="subject" name="subject" value="{{ $syllabus->subject }}" placeholder="e.g., Mathematics" required 
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="duration" name="duration" value="{{ $syllabus->duration }}" placeholder="e.g., 12 weeks" required 
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="topics" name="topics" rows="4" placeholder="Enter the main topics covered in this syllabus..."
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>{{ $syllabus->topics }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="objectives" name="objectives" rows="3" placeholder="What students should be able to achieve by the end of this term..."
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ $syllabus->objectives }}</textarea>
            </div>
